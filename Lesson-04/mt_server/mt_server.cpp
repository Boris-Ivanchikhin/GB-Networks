...
//
//  1. Добавьте в TCP-сервер возможность отдавать части файла по смещению и размеру.
//
//      P.S.
//      Запрос от TCP-клиента поступает виде строки “ путь\имя файла \nСмещение\nКоличество байт\n”;
...
public:
    ...
    // Отправка запрошенного фрагмента файла
    bool send_file_fragment(fs::path const& file_path, std::size_t offset, std::size_t size)
    {
        ...
        std::cout << "Sending file " << file_path << ", offset = " << offset << ", size = " << size << "..." << std::endl;

        // Перемещение указателя в файле на указанный offset
...
public:
    ...
    // Парсинг запроса вида "путь\имя файла \nСмещение\nКоличество байт\n".
    // Функция возвращает optional<кортеж из <пути к файлу, смещения и количества байтов для отправки>>
    std::optional<std::tuple<fs::path, std::size_t, std::size_t>> recv_request()
    {
        auto request = tsr_.get_request();
        if (!request.size())
            return std::nullopt;

        //std::string_view request_view(request);
        std::size_t offset_pos = request.find('\n');
        std::size_t size_pos = request.find('\n', offset_pos + 1);
        if (offset_pos == std::string_view::npos || size_pos == std::string_view::npos)
        {
            return std::nullopt;
        }

        std::size_t offset = std::stoul(std::string(request.substr(offset_pos + 1, size_pos - offset_pos - 1)));
        std::size_t size = std::stoul(std::string(request.substr(size_pos + 1)));

        //fs::path file_path(request.substr(0, offset_pos));
        auto cur_path = fs::current_path().wstring();
        auto file_path = fs::weakly_canonical(request.substr(0, offset_pos)).wstring();
        if (file_path.find(cur_path) == 0)
        {
            file_path = file_path.substr(cur_path.length());
        }

        return std::make_tuple(fs::weakly_canonical(cur_path + separ + file_path), offset, size);
    }
    ...
    bool send_file_fragment(fs::path const& file_path, std::size_t offset, std::size_t size)
    {
        ...
        return tsr_.send_file_fragment(file_path, offset, size);
    }
    ...
    bool process()
    {
        ...
        {
            auto [file_path, offset, size] = request.value();
            if (offset == 0 && size == 0)
            {
                // Запрошен весь файл
            ...
            {
                // Запрошен фрагмент файла
                result = send_file_fragment(file_path, offset, size);
            }
        }

        if (result)
            std::cout << "Sending file completed" << std::endl;
        else
            std::cerr << "Transceiver error" << std::endl;
